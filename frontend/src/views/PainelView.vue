<template>
  <div class="container my-4">
    <h2 class="text-center mb-4">Todas Reservas</h2>

    <loading-spinner :isLoading="isLoading" />

    <div v-if="reservas.length === 0" class="text-center text-muted">
      <p>O restaurante ainda não possui nenhuma reserva.</p>
    </div>
    ...
    <div v-else class="row g-3 overflow-auto reservas-container p-3">
      ...
      >
        <div 
          ...
          :class="[reserva.status === 'CANCELADA' ? 'card-disabled' : '', reserva.isPast && reserva.status !== 'CANCELADA' ? 'card-concluido' : '']"
        >
          <div 
            v-if="reserva.status === 'CANCELADA'" class="badge-cancelada"
          >
            Reserva Cancelada
          </div>
          <div class="card-body">
            ...
            <p class="card-text mb-0">
              <strong>Cliente:</strong> {{ reserva.usuario.nome }}
            </p>
            <p class="card-text">
            ...
            <button
              class="btn"
              :class="{'btn-danger': reserva.status !== 'CANCELADA' && !reserva.isPast, 'btn-secondary': reserva.isPast }"
              :disabled="reserva.status === 'CANCELADA' && reserva.isPast"
              @click="openDeleteModal(reserva)"
            >
              {{ reserva.isPast && reserva.status !== 'CANCELADA' ? '✅ Concluída' : 'Cancelar reserva' }}
            </button>
          </div>
        </div>
      </div>
  ...
  methods: {
    async fetchReservas() {
      ...
      try {
        ...
        this.isAdmin && console.error("Erro ao buscar reservas:", error);
      } finally {
    ...
    async deleteReserva() {
      ...
      try {
        ...
        this.isAdmin && console.error("Erro ao deletar reserva:", error);
